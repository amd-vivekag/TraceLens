...
class RocprofAnalyzer:
    ...
    def __init__(self, kernel_events: List[dict], memory_events: List[dict], 
                 api_events: List[dict], metadata: dict):
    ...
    def get_df_gpu_timeline(self) -> pd.DataFrame:
        ...
        data = [
            ...
            {'type': 'kernel', 'time ms': kernel_time_us / 1000.0, 
             'percent': (kernel_time_us / total_time_us * 100.0) if total_time_us > 0 else 0},
            {'type': 'memory', 'time ms': memory_time_us / 1000.0,
             'percent': (memory_time_us / total_time_us * 100.0) if total_time_us > 0 else 0},
            {'type': 'idle', 'time ms': idle_time_us / 1000.0,
             'percent': (idle_time_us / total_time_us * 100.0) if total_time_us > 0 else 0},
        ]
    ...
    def get_df_kernel_summary(self) -> pd.DataFrame:
        ...
        """
        if not self.kernel_events:
            return pd.DataFrame(columns=['name', 'Count', 'Total Kernel Time (µs)', 
                                        'Mean Kernel Time (µs)', 'Median Kernel Time (µs)',
                                        'Std Kernel Time (µs)', 'Min Kernel Time (µs)',
                                        'Max Kernel Time (µs)', 'Percentage (%)'])
        ...
        agg_dict = {
            'dur': ['count', 'sum', 'mean', 'median', 'std', 'min', 'max']
        }
        df_summary = df_kernels.groupby('name').agg(agg_dict)
        df_summary.columns = ['Count', 'Total Kernel Time (µs)', 'Mean Kernel Time (µs)',
                             'Median Kernel Time (µs)', 'Std Kernel Time (µs)',
                             'Min Kernel Time (µs)', 'Max Kernel Time (µs)']
        df_summary.reset_index(inplace=True)
        
        # Add percentage column
        total_time = df_summary['Total Kernel Time (µs)'].sum()
        df_summary['Percentage (%)'] = (df_summary['Total Kernel Time (µs)'] / total_time * 100.0) if total_time > 0 else 0
        df_summary['Cumulative Percentage (%)'] = df_summary['Percentage (%)'].cumsum()
        
        # Sort by total time descending
        df_summary.sort_values(by='Total Kernel Time (µs)', ascending=False, inplace=True)
        df_summary.reset_index(drop=True, inplace=True)
        
        # Convert to milliseconds for total time column
        df_summary['Total Kernel Time (ms)'] = df_summary['Total Kernel Time (µs)'] / 1000.0
    ...
    def get_df_kernel_summary_by_category(self) -> pd.DataFrame:
        ...
        if not self.kernel_events:
            return pd.DataFrame(columns=['op category', 'Count', 'total_direct_kernel_time_ms',
        ...
        def categorize_kernel(name: str) -> str:
            ...
            if 'gemm' in name_lower or 'matmul' in name_lower or 'cijk' in name_lower:
                ...
                return 'Convolution'
            elif 'norm' in name_lower or 'batch_norm' in name_lower or 'layer_norm' in name_lower:
        ...
        # Group by category
        df_summary = df_kernels.groupby('op category').agg({
            'dur': ['count', 'sum']
        })
        ...
        df_summary['total_direct_kernel_time_ms'] = df_summary['total_direct_kernel_time_us'] / 1000.0
        df_summary.drop(columns=['total_direct_kernel_time_us'], inplace=True)
        
        # Add percentages
        total_time = df_summary['total_direct_kernel_time_ms'].sum()
        df_summary['Percentage (%)'] = (df_summary['total_direct_kernel_time_ms'] / total_time * 100.0) if total_time > 0 else 0
        df_summary['Cumulative Percentage (%)'] = df_summary['Percentage (%)'].cumsum()
        
        # Sort by total time descending
        df_summary.sort_values(by='total_direct_kernel_time_ms', ascending=False, inplace=True)
        df_summary.reset_index(drop=True, inplace=True)
    ...
    def get_df_short_kernels(self, threshold_us: float = 10.0) -> pd.DataFrame:
        ...
        if not self.kernel_events:
            return pd.DataFrame(columns=['name', 'Short Kernel count', 'Short Kernel duration (µs) sum',
                                        'Short Kernel duration (µs) mean', 'Short Kernel duration (µs) percent of total time'])
        ...
        df_summary = df_short.groupby('name').agg({
            'dur': ['count', 'sum', 'mean']
        })
        df_summary.columns = ['Short Kernel count', 'Short Kernel duration (µs) sum', 'Short Kernel duration (µs) mean']
        df_summary.reset_index(inplace=True)
        
        # Calculate percentage of total time
        total_time = sum(event['dur'] for event in self.kernel_events)
        df_summary['Short Kernel duration (µs) percent of total time'] = \
            (df_summary['Short Kernel duration (µs) sum'] / total_time * 100.0) if total_time > 0 else 0
        
        # Sort by total time descending
        df_summary.sort_values(by='Short Kernel duration (µs) sum', ascending=False, inplace=True)
        df_summary.reset_index(drop=True, inplace=True)
        
        return df_summary
    
    def get_df_short_kernel_histogram(self, threshold_us: float = 10.0, bins: int = 100) -> pd.DataFrame:
    ...
    def get_df_kernel_details(self, topk: Optional[int] = None) -> pd.DataFrame:
        ...
        columns = ['name', 'Kernel duration (µs)', 'stream', 'dispatch_id', 
